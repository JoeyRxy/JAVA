...
锁 就像公共厕所的门上的锁，多个线程想要使用这个厕所，总有一个人抢先“**获取**”了锁，然后进门之后他会先锁门，防止别人使用这个坑位。
...
`synchronized`关键字修饰成员方法`f1()`，表示执行该方法需要获取当前对象的锁。对于同一个对象`a`来说，如果多个线程调用该方法`a.f1()`，那么只有一个线程的`a.f1()`在运行直到释放这个锁。

修饰静态方法，表示执行该方法`f2()`需要获取当前类的锁。如果多个线程想要调用`A.f2()`，那么只有一个线程能得到执行的锁并运行，直到释放这个锁。

对于同步代码块来说是同样的道理：
- 可以使用`synchrnized(this)`。需要获取当前对象`this`的锁来运行这个代码块。如果多个线程想运行这个代码块，必定是通过类的对象`a`调用，那么线程需要争夺对象`a`的锁。
